##############################
##  <CONTRACTS>             ##
##  These are items which   ##
##  we are fully dependent  ##
##  upon and represent an   ##
##  agreement set forth     ##
##  with a 'provider.'      ##
##############################

# Move to data resources for accuracy
# pcf_infrastructure_subnet               = "10.44.32.32/27"
# pcf_pas_subnet                          = "10.44.40.0/22"
# pcf_services_subnet                     = "10.44.44.0/22"
# pcf_pks_subnet                          = "10.44.36.0/22"

###################
##  SPOKE DEETS  ##
###################
# env_name makes up a large part of the resources created by this script, but also
# represents the prefix of our pre-defined resource group (i.e. env_name-rg1).
env_name                                = "demo-uscentral-pcf"
pcf_resource_group_name                 = "demo-uscentral-pcf-rg1"
pcf_spoke_resource_group                = "etg-uscentral-etgci-pcfspoke-rg1"
pcf_spoke_vnet                          = "etg-uscentral-etgci-pcf-vnet1"
pcf_infra_name                          = "infra"
pcf_svcs_name                           = "services"
pcf_depl_name                           = "deploy"
pcf_pks_network                         = "pks-services"
ntp_server                              = "10.44.32.133"
dns_server                              = "10.44.32.135"

################
#    OpsMan    #
################
# We could use a local source image here instead of the URL!
#ops_manager_image_uri                   = "https://opsmanagereastus.blob.core.windows.net/images/ops-manager-2.6.11-build.210.vhd"
ops_manager_image_uri                   = "https://opsmanagereastus.blob.core.windows.net/images/ops-manager-2.5.19-build.271.vhd"
#Add for SAML
# saml_idp_metadata                       = "https://login.microsoftonline.com/REPLACE_ME/federationmetadata/2007-06/federationmetadata.xml?appid=3c01e1d9-fecd-4e47-b85b-d2d895963280"
# saml_rbac_admin_pcf_resource_group_name = "REPLACE_ME"
# saml_rbac_groups_attribute              = "http://schemas.microsoft.com/ws/2008/06/identity/claims/groups"

##############################
##  </CONTRACTS>            ##
##############################

##############################
##  GENERAL VARS            ##
##############################

# Short name is derived from env_name, but obviously different and used primarily for storage accounts
# Must be unique
env_short_name                          = "azpcf"

# Not what you think. Adds another subdomain onto the dns_suffix
dns_subdomain                           = ""

location                                = "centralus"
dns_suffix                              = "turtleisland.net"
vm_admin_username                       = "ubuntu"
cf_storage_account_name                 = "sa"
decryption-passphrase                   = "DecryPtM3!"
director_hostname                       = "director.turtleisland.net"

##########################
#        SSL KEYS        #
##########################
ssl_cert = <<SSL_CERT
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
SSL_CERT

ssl_private_key = <<SSL_KEY
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
SSL_KEY
